...
const setUserIncome = async (userId, userIncome, exists) => {
  if (!exists) {
    try {
      await mysql.query('INSERT INTO ranking (user_id, income) VALUES (?, ?)', [
        userId,
        userIncome,
      ])
    } catch (error) {
    ...
    try {
      await mysql.query('UPDATE ranking SET income=? WHERE user_id=?', [
        userIncome,
        userId,
      ])
    } catch (error) {
...
const run = async () => {
  ...
  users.forEach(async user => {
    ...
    const [userBuildings] = await mysql.query('SELECT buildings.id, buildings.quantity, research.level AS optimizeLevel FROM buildings LEFT OUTER JOIN users ON users.id = buildings.user_id LEFT OUTER JOIN research ON users.id = research.user_id WHERE users.id = ?', [user.id])

    let userTotalIncome
    if (userBuildings.length) {
      // Calculate the total income
      userTotalIncome = userBuildings.map(({ id: buildingId, quantity, optimizeLevel }) => {
