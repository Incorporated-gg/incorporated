...
module.exports = app => {
  app.get('/v1/contests/spying', async function(req, res) {
    if (!req.userData) {
    ...
    const [[feverContest]] = await mysql.query(
      "SELECT id FROM contests WHERE name='spying' AND started_at IS NOT NULL AND ended_at IS NULL"
    )
    const [
      feverScoreboards,
    ] = await mysql.query(
    ...
    const spying = await Promise.all(
