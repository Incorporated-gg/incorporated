...
export async function getResearchScoreboard(weekFirstServerDay) {
  ...
  const topUsers = await mysql.query(
    `SELECT user_id,
    (
      (SELECT researchs_count FROM users_daily_log udl2 WHERE server_day>=? AND server_day<? AND udl2.user_id=users_daily_log.user_id ORDER BY server_day DESC LIMIT 1)
      -
      (SELECT researchs_count FROM users_daily_log udl2 WHERE server_day>=? AND server_day<? AND udl2.user_id=users_daily_log.user_id  ORDER BY server_day LIMIT 1)
    ) as diff
