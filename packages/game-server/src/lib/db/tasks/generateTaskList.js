  ...
  for (let taskID = 1; taskID <= totalTasks; taskID++) {
    ...
    switch (type) {
      case 'cyclic_build': {
        ...
        const amount = Math.floor(Math.pow(2, 6 - buildingID) * ((typeCounter - 1) * 0.167 + 1)) // Equivalent to +1 hotel every 6 loops (+0,1665 hotels every loop), starting with 1 cinema.
