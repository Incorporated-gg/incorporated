      ...
      <>
        <div>
          <div className={buildingStyles.buildingStat}>
            <div className={`titleText shadow ${buildingStyles.buildingStatTitle}`}>PRI</div>
            <div className={buildingStyles.buildingStatValue}>{timeToRecoverInvestment} días</div>
          </div>
        </div>
        <Container onClick={buyResearchClicked} outerClassName={cardStyles.button} disabled={!canBuy}>
          ...
          <h2 className={`titleText shadow pascal ${buildingStyles.buildingAction}`}>{'MEJORAR'}</h2>
        </Container>
      </>
    </Card>
  )
